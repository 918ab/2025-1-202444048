...
namespace Week09Homework
{
    ...
    class Student : Member, IFile
    {
        ...
        {
            get { return $"{Number}|{Name}|{Dept.Code}|{BirthInfo:yyyy,MM,dd}|{AdvisorNumber}|{Year}|{(int)Class}|{(int)RegStatus}"; }
        }
        ...
        public static Student Restore(string record, Department[] departments)
        {
            ...
            try{
                ...
                if (sdata.Length == 10){
                    //0 학번 1 이름 2 학과 3 생일 4 지도교수 5 학년 6 반 7 등록상태 8 주소 9 연락처
                    ...
                    if (Enum.TryParse<YEAR>(sdata[6], out var tempclass)){
                        student.Class = (CLASS) tempclass;
                    } else{
                        student.Class = CLASS.A;
                    }

                    if (Enum.TryParse<YEAR>(sdata[7], out var tempregstt)){
